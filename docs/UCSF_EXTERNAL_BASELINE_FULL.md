...
评价默认沿用原项目概率阈值0.33、26连通、一对一最大Dice匹配、预测和GT组件最小10体素。结果同时报告病例Dice、HD95、1 mm表面Dice、病灶召回率/精确率，以及漏检记0的逐GT病灶Dice。测试集阈值不得根据最终结果再调；如需调阈值，应只在固定验证集完成，然后把选定阈值用于一次测试。
