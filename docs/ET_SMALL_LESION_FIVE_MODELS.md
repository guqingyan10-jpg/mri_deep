# Five-model small ET lesion evaluation
...
voxels, and a training-defined small stratum of 10–44 voxels. There should be
31 small GT ET lesions. `--strata-json /path/to/et_training_lesion_strata.json`
optionally verifies the frozen training thresholds. The script stops if the
case count, small-lesion count, or small-lesion GT identities differ.

Outputs are written to `et_small_lesion_five_models_results/`:

- `summary.csv`: matched lesion Dice, GT-anchored lesion Dice, recall, and
  detected/missed counts for each model.
- `small_lesions.csv`: one row per GT small lesion per model, with missed
  lesions scored as zero in `gt_anchored_dice`.
- `test_cases.csv`: the evaluated case IDs.

Matched lesion Dice averages successful one-to-one matches only. GT-anchored
lesion Dice averages across all 31 GT small lesions, assigning zero to each
unmatched lesion. The existing formal seed-55 results provide a check: the
