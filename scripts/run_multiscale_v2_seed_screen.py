"""Run the identity-start MultiScaleContext V2 experiment for selected seeds."""
...
def build_jobs(seeds: Sequence[int], output_root: Path,
               epochs: int, lr: float) -> list[MultiScaleV2TrainingJob]:
    jobs = []
    for seed in seeds:
        checkpoint_dir = (
        ...
        jobs.append(MultiScaleV2TrainingJob(
            seed=seed,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train identity-start multi-scale context V2 models",
    )
    ...
    for job in jobs:
        ...
        print(f"\n[hf_concat_boundary_w0.1_multiscale_v2 | seed={job.seed}]")
